...
#### Motivation
Building a 3D scene is a lot easier when you can see the node hierarchy and tweak transforms interactively. This project is a small desktop not full game engine, just a clean scene graph you can edit around in and a live 3D viewport to see what's happening.
...
- **Edit transforms**: drag Position (Vec3), Rotation (Quaternion), and Scale (Vec3) for any node.
...
2. **Inspector panel** — editable fields for whatever is selected: tag, transform (position/rotation/scale), and type-specific stuff (projection type, FOV, light color, etc.). Non-root nodes also get a Delete button.
3. **Viewport panel** — resizable live 3D view. Right-drag to orbit, middle-drag to pan, scroll to zoom.

The viewport renders:
- A reference grid with a red X-axis line and a blue Z-axis line.
...
#### Testing
Unit testing done with xUnit, to test core libraries correctness.
